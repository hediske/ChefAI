...
class ChromaStore:
    ...
    def add_documents(self, docs: List[Document]):
        ...
        print(f"Successfully Added {len(docs)} documents to the vectorstore")
    
    def add_recipe_chunks(self, chunks: List[Dict]):
        documents = []
        print(f"Adding {len(chunks)} recipe chunks to documents list")
        for chunk in chunks:
            ...
            if chunk["type"] == "metadata":
                metadata.update({
                    "picture_link": chunk.get("picture_link"),
                    "is_metadata": True
                })
            elif chunk["type"] == "ingredients":
                metadata["group"] = chunk["group"]
            elif chunk["type"] == "instructions":
                metadata["step"] = chunk["step"]
